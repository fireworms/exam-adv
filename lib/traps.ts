...
export function normalizeTrapData(subject: string, json: Record<string, unknown>): TrapData[] {
  ...
  if (subject === 'infosec') {
    ...
    // 대표(title)는 참인 문장을 둔다 — 틀린 문장이 먼저 각인되는 것을 방지.
    // 정답 필드는 주어가 빠진 단편이라(예: "비대칭키 (키교환)") 주어를 살린 완성 문장으로 작성.
    // 틀린생각은 ❌로 아래에 흔한 오해로만 노출.
    const titles: string[] = [
      '디피-헬만은 비대칭 방식의 키 교환(키 합의) 프로토콜이다',
      'BLP는 기밀성 모델이다 (무결성은 Biba)',
      'ECC는 타원곡선 기반의 비대칭키 암호다',
      'FRR은 오거부율(정당한 사용자 거부)이다 (오수락률은 FAR)',
      'Pharming은 DNS 캐시·호스트파일 조작 공격이다',
      'CC EAL은 7등급이 가장 높다 (1이 최저)',
      '위치정보법 소관 부처는 방송통신위원회다',
      '개인정보 처리방침은 공개 의무가 있다',
      '개인정보는 익명처리를 우선하고, 불가능하면 가명처리한다',
      '정보주체는 자동화된 결정에 대해 거부권·설명요구권을 가진다',
      '개인정보 제3자 제공의 근거는 정보주체와의 계약(동의)이다',
      '개인정보 파기 기준은 대통령령으로 정한다',
      'Kerberos v5는 nonce를 사용해 재생공격을 방어한다',
      'ESP는 기밀성·무결성·(선택적) 인증을 제공한다',
      'AH는 인증·무결성만 제공한다 (기밀성 없음)',
      'SHA-512는 80라운드·1024bit 블록을 사용한다',
      'AES 마지막 라운드는 MixColumns 연산을 생략한다',
      '순서번호는 TCP만 사용한다 (UDP에는 없음)',
      'IPv6는 브로드캐스트가 폐지되고 애니캐스트를 지원한다',
      'HTTP GET Flooding은 DoS 공격이다 (입력값 검증과 무관)',
    ]
    return raw.map((t, i) => ({
      rank: i + 1,
      title: titles[i] ?? t.정답,
